<!-- Create Client Modal -->
<flux:modal name="create-client" :show="$showModal" focusable class="max-w-2xl">
    <form wire:submit="createClient" class="space-y-6">
        <div class="p-6">
            <flux:heading size="lg">{{ __('Create New Client') }}</flux:heading>
            <flux:subheading>{{ __('Add a new client to your organization') }}</flux:subheading>
        ...
        <div class="flex justify-end space-x-2 rtl:space-x-reverse px-6 py-4 bg-gray-50 border-t">
            ...
            <flux:button wire:click="createClient" variant="primary">{{ __('Create Client') }}</flux:button>
        </div>
    </form>
</flux:modal>
