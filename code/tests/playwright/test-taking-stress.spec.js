...
test("user can complete test", async ({ page }) => {
    ...
    const testButton = page
        .locator('a[href*="/test"], button:has-text("Start")')
        .first();
    ...
    if (await testButton.isVisible({ timeout: 5000 })) {
        ...
        if (currentUrl.includes("/test")) {
            ...
            console.log(
                "This test requires a test to be assigned to the user in the dashboard"
            );
        }
    } else {
        ...
        console.log(
            "Please assign a test to the user in the application first"
        );
    }
});
 ...
 */
test("user completes entire test and reaches results page", async ({
    page,
}, testInfo) => {
    ...
    if (!(await testButton.isVisible({ timeout: 5000 }))) {
        throw new Error(
            "No test available on dashboard for the user; cannot proceed with full test flow"
        );
    }
    ...
    while (questionCount < maxQuestions) {
        ...
        try {
            ...
            await button.click();
            console.log(
                `Question ${questionCount}: Clicked ${
                    random > 0.5 ? "LIKE" : "DISLIKE"
                }`
            );
            ...
            await test.step("wait for image load", async () => {
            ...
            console.log(
                `Question ${questionCount}: Button did not become enabled within 5s; will wait and retry. (${err.message})`
            );
            throw new Error(
                `Stalled on question ${questionCount}: ${err.message}`
            );
        }
    }
    ...
    if (page.url().includes("/test-result")) {
        ...
        throw new Error(
            `Test did not complete normally. Answered ${questionCount} questions. Current URL: ${page.url()}`
        );
    }
});
